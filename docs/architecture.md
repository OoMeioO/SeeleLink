...
## 10. WebBridge + Web UI 多端同步

```
┌─────────────────┐     WebSocket      ┌─────────────────┐
│  浏览器 Web UI   │ ◄───────────────►  │  WebBridge (:9382) │
│  http://:9383   │                    │  (Electron Main)   │
└─────────────────┘                    └────────┬────────┘
                                                │
                                                │ IPC
                                       ┌────────▼────────┐
...
多端同步机制:
1. 连接变更时 → broadcastConnectionsChanged() 通知所有 WebBridge 客户端
2. Web UI 刷新时 → seelelink:connected 事件触发自动重载连接
3. 连接变更通知 → seelelink:connections-changed 事件同步状态
```

| Port | Service | Description |
|------|---------|-------------|
| 9382 | WebBridge | WebSocket relay, browser remote access |
| 9383 | Web UI | HTTP server, browser UI access |
